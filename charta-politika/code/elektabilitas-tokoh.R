...
df <- read.csv("elektabilitas-tokoh.csv")
glimpse(df)

df <- df %>% 
  mutate(nama = forcats::fct_rev(forcats::fct_inorder(nama)))
...
final_plot <- df %>% 
  mutate(place = if_else(row_number() == 1, 1, 0),
         perc = paste(" ", perc, " ")) %>% 
ggplot(aes(x = elektabilitas, y = nama, fill = color)) +
  ...
  labs(
    ...
    subtitle = "<span style='color:#ff0000'>Elektabilitas Tokoh Menurut Charta Politika</span> 
    <br>Survei yang dilakukan oleh Charta Politika periode 8-16 Desember menemukan 
    <br>Ganjar Pranowo menempati posisi elektabilitas tertinggi dari 10 tokoh lain",
...
ragg::agg_png(here("elektabilitas-tokoh.png"), res = 100, width = 650, height = 433, units = "px")
final_plot
grid.lines(
  x = c(0, 1),
  y = 1,
